...
export const Advanced = ({ open, setOpenModalAdvanced, types, setCountChekedFilter, statusData, handleClick }: Props) => {
    ...
    useEffect(() => {
        const result = [checkFecha, checkType, checkStatus, checkIdMovement].filter((item) => item)
        setCountChekedFilter(result.length)
    }, [checkFecha, checkType, checkStatus, checkIdMovement])
    ...
    const handleSubmit = () => {
        handleClick('CUSTOM_FILTER', { dateObj, type, status, game: operation })
