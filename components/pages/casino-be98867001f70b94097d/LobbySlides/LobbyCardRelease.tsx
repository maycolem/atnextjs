...
import { onClose } from '@states/features/slices/CasinoModalSlice'
...
export const LobbyCardRelease = ({ lobby, pathname, section }: Props) => {
    ...
    const handlePlayOnMobile = async () => {
    ...
    return (
        <StyledCard $column={lobbyTagSlice(lobby_tag, 0, 1)} $row={lobbyTagSlice(lobby_tag, 2, 3)} title={name}>
            <img onClick={handlePlayOnMobile} alt="" src={`${process.env.REACT_APP_WEB_BASE}/${logo}`} loading="lazy" />
            <WrapperAccions>
                {
                    !mobileL &&
                        <ButtonS onClick={handlePlay} title={`jugar ${name}`} variant="contained">
                            <div>
                                <span className="play">Jugar</span>
                            </div>
                        </ButtonS>
                }
            </WrapperAccions>
